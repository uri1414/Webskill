// Screen 5: Client Confirmation. Read-only view across Request + relation +
// Appointment. RLS guarantees the client only ever sees their own.
...
export default async function ClientRequestPage({ params }: { params: { id: string } }) {
  ...
  type Appt = { id: string; title: string | null; status: string; starts_at: string | null };
  let appointment: Appt | null = null;
  if (rel?.entity_id) {
    const { data } = await supabase
      .from("appointments")
      .select("id, title, status, starts_at")
      .eq("id", rel.entity_id as string)
      .single();
    appointment = (data as unknown as Appt) ?? null;
  }
